...
class Interface:
    ...
    def set_device_serial_number(self, serial_number):
        request = Message(
            [0xAA, 0xAA], 2, 0, True, False, [serial_number], direction="out"
        )
        return self.send(request)
    ...
    def set_device_name(self, device_name):
        request = Message(
            [0xAA, 0xAA], 2, 1, True, False, [device_name], direction="out"
        )
        return self.send(request)
    ...
    def set_homing_parameters(self, x, y, z, r, queue=True):
        request = Message(
            [0xAA, 0xAA], 2, 30, True, queue, [x, y, z, r], direction="out"
        )
        return self.send(request)
    ...
    def set_auto_leveling(self, enable, accuracy, queue=True):
        request = Message(
            [0xAA, 0xAA], 2, 32, True, queue, [enable, accuracy], direction="out"
        )
        return self.send(request)
    ...
    def set_end_effector_params(self, bias_x, bias_y, bias_z):
        request = Message(
            [0xAA, 0xAA], 2, 60, True, False, [bias_x, bias_y, bias_z], direction="out"
        )
        return self.send(request)
    ...
    def set_jog_joint_params(self, velocity, acceleration, queue=True):
        request = Message(
            [0xAA, 0xAA], 2, 70, True, queue, velocity + acceleration, direction="out"
        )
        return self.send(request)
    ...
    def set_jog_coordinate_params(self, velocity, acceleration, queue=True):
        request = Message(
            [0xAA, 0xAA], 2, 71, True, queue, velocity + acceleration, direction="out"
        )
        return self.send(request)
    ...
    def set_jog_command(self, jog_type, command, queue=True):
        request = Message(
            [0xAA, 0xAA], 2, 73, True, queue, [jog_type, command], direction="out"
        )
        return self.send(request)
    ...
    def set_sliding_rail_jog_params(self, velocity, acceleration, queue=True):
        request = Message(
            [0xAA, 0xAA], 2, 74, True, queue, [velocity, acceleration], direction="out"
        )
        return self.send(request)
    ...
    def set_point_to_point_joint_params(self, velocity, acceleration, queue=True):
        request = Message(
            [0xAA, 0xAA], 2, 80, True, queue, velocity + acceleration, direction="out"
        )
        return self.send(request)
    ...
    def set_point_to_point_jump_params(self, jump_height, z_limit, queue=True):
        request = Message(
            [0xAA, 0xAA], 2, 82, True, queue, [jump_height, z_limit], direction="out"
        )
        return self.send(request)
    ...
    def set_point_to_point_command(self, mode, x, y, z, r, queue=True):
        request = Message(
            [0xAA, 0xAA], 2, 84, True, queue, [mode, x, y, z, r], direction="out"
        )
        return self.send(request)
    ...
    def set_point_to_point_sliding_rail_params(
        ...
        request = Message(
            [0xAA, 0xAA], 2, 85, True, queue, [velocity, acceleration], direction="out"
        )
        return self.send(request)

    def set_point_to_point_sliding_rail_command(self, mode, x, y, z, r, l, queue=True):
        request = Message(
            [0xAA, 0xAA], 2, 86, True, queue, [mode, x, y, z, r, l], direction="out"
        )
        return self.send(request)
    ...
    def set_point_to_point_po_command(self, mode, x, y, z, r, queue=True):
        request = Message(
            [0xAA, 0xAA], 2, 88, True, queue, [mode, x, y, z, r], direction="out"
        )
        return self.send(request)
    ...
    def set_point_to_point_sliding_rail_po_command(
        ...
        request = Message(
            [0xAA, 0xAA], 2, 89, True, queue, [ratio, address, level], direction="out"
        )
        return self.send(request)
    ...
    def set_continous_trajectory_command(self, mode, x, y, z, velocity, queue=True):
        request = Message(
            [0xAA, 0xAA], 2, 91, True, queue, [mode, x, y, z, velocity], direction="out"
        )
        return self.send(request)
    ...
    def set_continous_trajectory_laser_engraver_command(
        ...
        request = Message(
            [0xAA, 0xAA], 2, 92, True, queue, [mode, x, y, z, power], direction="out"
        )
        return self.send(request)
    ...
    def wait(self, milliseconds, queue=True):
        request = Message(
            [0xAA, 0xAA], 2, 110, True, queue, [milliseconds], direction="out"
        )
        return self.send(request)
    ...
    def set_io_multiplexing(self, address, multiplex, queue=True):
        request = Message(
            [0xAA, 0xAA], 2, 130, True, queue, [address, multiplex], direction="out"
        )
        return self.send(request)
    ...
    def set_io_do(self, address, level, queue=True):
        request = Message(
            [0xAA, 0xAA], 2, 131, True, queue, [address, level], direction="out"
        )
        return self.send(request)
    ...
    def set_extended_motor_velocity(self, index, enable, speed, queue=True):
        request = Message(
            [0xAA, 0xAA], 2, 135, True, queue, [index, enable, speed], direction="out"
        )
        return self.send(request)
    ...
    def set_color_sensor(self, index, enable, port, version, queue=True):
        request = Message(
            [0xAA, 0xAA], 2, 137, True, queue, [enable, port, version], direction="out"
        )
        return self.send(request)
    ...
    def set_ir_switch(self, index, enable, port, version, queue=True):
        request = Message(
            [0xAA, 0xAA], 2, 138, True, queue, [enable, port, version], direction="out"
        )
        return self.send(request)
    ...
    def set_wifi_address(self, index, use_dhcp, a, b, c, d):
        request = Message(
            [0xAA, 0xAA], 2, 153, True, False, [use_dhcp, a, b, c, d], direction="out"
        )
        return self.send(request)
    ...
    def set_wifi_netmask(self, index, a, b, c, d):
        request = Message(
            [0xAA, 0xAA], 2, 154, True, False, [a, b, c, d], direction="out"
        )
        return self.send(request)
    ...
    def set_wifi_gateway(self, index, use_dhcp, a, b, c, d):
        request = Message(
            [0xAA, 0xAA], 2, 155, True, False, [use_dhcp, a, b, c, d], direction="out"
        )
        return self.send(request)
    ...
    def set_wifi_dns(self, index, use_dhcp, a, b, c, d):
        request = Message(
            [0xAA, 0xAA], 2, 156, True, False, [use_dhcp, a, b, c, d], direction="out"
        )
        return self.send(request)
    ...
    def stop_queue(self, force=False):
        request = Message(
            [0xAA, 0xAA], 2, 242 if force else 241, True, False, [], direction="out"
        )
        return self.send(request)
